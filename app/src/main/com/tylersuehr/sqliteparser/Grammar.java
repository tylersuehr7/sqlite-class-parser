 ...
 * Created by tyler
 *
 * Transforms Grammar words from singular to plural form.
 *
 * Examples:
 * Grammar.plural("word") = "words";
 * Grammar.plural("cat", 1) = "cat";
 * Grammar.plural("cat", 2) = "cats";
 */
class Grammar extends Inflector {
    private static final String[] CATEGORY_EX_ICES = { "codex", "murex", "silex", };
    private static final String[] CATEGORY_IX_ICES = { "radix", "helix", };
    private static final String[] CATEGORY_UM_A = { "bacterium", "agendum", "desideratum",
            "erratum", "stratum", "datum", "ovum", "extremum", "candelabrum", };

    // Always us -> i
    private static final String[] CATEGORY_US_I = { "alumnus", "alveolus", "bacillus",
            "bronchus", "locus", "nucleus", "stimulus", "meniscus", "thesaurus", };

    private static final String[] CATEGORY_ON_A = { "criterion", "perihelion", "aphelion",
    ...
    private static final String[] CATEGORY_O_I = { "solo", "soprano", "basso",
            "alto", "contralto", "tempo", "piano", "virtuoso", };
    ...
    private static final String[] CATEGORY_EX_EXES = { "apex", "latex", "vertex",
            "cortex", "pontifex", "vortex", "index", "simplex"
    };
    ...
    private static final String[] CATEGORY_MAN_MANS = { "human", "Alabaman", "Bahaman",
            "Burman", "German", "Hiroshiman", "Liman", "Nakayaman", "Oklahoman", "Panaman",
            "Selman", "Sonaman", "Tacoman", "Yakiman", "Yokohaman", "Yuman"
    };

    private static Grammar inflector = new Grammar();


    public Grammar() {
        this(MODE.Grammar_ANGLICIZED);
    }

    public Grammar(MODE mode) {
        ...
        if (mode == MODE.Grammar_ANGLICIZED) {
            // Anglicized plural
            irregular(new String[][] {
                    { "beef", "beefs" },
                    { "brother", "brothers" },
        ...
        } else if (mode == MODE.Grammar_CLASSICAL) {
            // Classical plural
        ...
        if (mode == MODE.Grammar_CLASSICAL) {
            rule(new String[][]{
                    { "trix$", "trices" },
                    { "eau$", "eaux" },
                    { "ieu$", "ieux" },
    ...
    public static void setMode(MODE mode) {
        Grammar newInflector = new Grammar(mode);
        inflector = newInflector;
    }


    public enum MODE {
        Grammar_ANGLICIZED, Grammar_CLASSICAL
    }
}